...
1. 实现跨进程的互斥同步:目前使用的全局表 table 是进程内的静态变量，每个进程会有一份。
而这些列表是使用 MAP_PRIVATE 映射到内存，但是 MAP_PRIVATE 是建立私有拷贝，不能同步到文件？如何测试这个论断的正确性？此外所有的锁都是进程内部 pthread_mutex_t（tabmsg.hMutex），未放在共享内存区。