...
void CThreadPool::clearMsgRecvQueue()
{
	...
	//尾声阶段，需要互斥？该退的都退出了，该停止的都停止了，应该不需要了
...
bool CThreadPool::Create(int threadNum)
{
	...
	{
		m_threadVector.push_back(pNew = new ThreadItem(this));             //创建一个新线程对象并保存到容器中         
...
void* CThreadPool::ThreadFunc(void* threadData)
{
	...
	while (true)
	{
		...
		//因为：pthread_cond_wait()是个值得注意的函数，调用一次pthread_cond_signal()可能会唤醒多个【惊群】【官方描述是至少一个pthread_cond_signal在多处理器上可能同时唤醒多个线程】
		...
		//能走下来的，必然是拿到了真正的消息队列中的数据，或者 m_shutdown == true
...
void CThreadPool::StopAll()
{
	...
	//唤醒等待该条件【卡在pthread_cond_wait()的】的所有线程，一定要在改变条件状态以后再给线程发信号
